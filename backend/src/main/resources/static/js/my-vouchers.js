...
function render(list){
  ...
  c.innerHTML=list.map(function(v){
    ...
    if(v.status===0){var exp=v.expireTime?new Date(v.expireTime)>new Date():true;if(exp){useBtn='<span class="vcard-use-btn" onclick="useVoucher('+v.id+')">去使用</span>';}else{statusHtml='<span class="vcard-status expired">已过期</span>';}}
...
function useVoucher(id){api.post('/user/vouchers/use/'+id).then(function(r){if(r.data.code===200){showToast('使用成功');loadVouchers();}else{showToast(r.data.msg);}});}
